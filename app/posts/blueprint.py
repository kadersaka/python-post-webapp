...
def post_create():
    ...
    if(request.method == 'POST'):
        ...
        return redirect('/posts/post_create.html', form=form)
...
def posts_list():
    ...
    if q:
        ...
        posts = Post.querry.order_by(Post.created.desc())
...
def posts_detail(slug):
    post = Post.querry.filter(Post.slug == slug).first_or_404()
    return render_template('posts/post_detail.html', post = post)

@posts.route('/tags/<slug>')
def tag_detail(slug):
    tag = Tag.query.all().filter(Tag.slug == slug).first_or_404()
